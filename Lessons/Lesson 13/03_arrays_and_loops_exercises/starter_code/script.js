...
let thePlanets = ["Mercury", "Venus", "Earth", "Mars", "Jupiter", "Saturn", "Uranus", "Neptune", "Pluto"];

// Remove the last planet from the array (sorry Pluto)
thePlanets.pop()

// Add a new planet to the end of the array
thePlanets.push("Bonus Planet");

// Print your new array to the dom
document.querySelector("#exercise-two").textContent = thePlanets;
...
for (let s = 0; s < snacks.length; s++) {
  if (snacks[s] === "popcorn") {
    document.querySelector("#exercise-four").textContent = `My favorite snack is ${snacks[s]}!`;
  } 
}
